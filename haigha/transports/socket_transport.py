...
class SocketTransport(Transport):
    ...
    def read(self, timeout=None):
        ...
        try:
            ...
            if timeout:
                ...
                self._sock.settimeout(None)
            data = self._sock.recv(self._sock.getsockopt(socket.SOL_SOCKET,
                                                         socket.SO_RCVBUF))

            if len(data):
                if self.connection.debug > 1:
                    self.connection.logger.debug('read %d bytes from %s' %
                                                 (len(data), self._host))
                if len(self._buffer):
            ...
            self.connection.logger.exception('error reading from %s' %
                                             (self._host))

        self.connection.transport_closed(msg='error reading from %s' %
                                         (self._host))
        if e:
            raise
    ...
    def write(self, data):
        ...
        try:
            ...
            if self.connection.debug > 1:
                self.connection.logger.debug('sent %d bytes to %s' %
                                             (len(data), self._host))
            ...
            self.connection.logger.exception('error writing to %s' %
                                             (self._host))

        self.connection.transport_closed(msg='error writing to %s' %
                                         (self._host))
