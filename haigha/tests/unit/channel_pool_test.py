...
class ChannelPoolTest(Chai):
    ...
    def test_get_channel_when_two_free_and_one_closed(self):
        ...
        expect(cp._free_channels.pop).returns(ch1).\
            side_effect(super(Set, cp._free_channels).pop)
        expect(cp._free_channels.pop).returns(ch2).\
            side_effect(super(Set, cp._free_channels).pop)
