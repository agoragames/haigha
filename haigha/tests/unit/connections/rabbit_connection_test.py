...
class RabbitConnectionTest(Chai):
    ...
    def test_init(self):
        with expect(mock(rabbit_connection, 'super')).\
                args(is_arg(RabbitConnection), RabbitConnection).returns(mock()) as c:
...
class RabbitExchangeClassTest(Chai):
    ...
    def test_bind_with_args(self):
        ...
        expect(self.klass.channel.add_synchronous_cb).\
            args(self.klass._recv_bind_ok)
        expect(mock(rabbit_connection, 'MethodFrame')).\
            args(42, 40, 30, w).returns('frame')
        expect(self.klass.send_frame).args('frame')
...
class RabbitBasicClassTest(Chai):
    ...
    def test_publish_when_not_confirming(self):
        ...
        with expect(mock(rabbit_connection, 'super')).args(
                is_arg(RabbitBasicClass),
                RabbitBasicClass).returns(mock()) as klass:
...
class RabbitConfirmClassTest(Chai):
    ...
    def test_init(self):
        assert_equals({11: self.klass._recv_select_ok},
                      self.klass.dispatch_map)
